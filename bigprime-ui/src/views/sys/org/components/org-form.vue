<template>
  <div class="ds-form">
    <tiny-form ref="formRef" overflow-title :model="formData" :rules="formRules" label-width="100px">
      ...
      <tiny-row>
        <tiny-col :span="6">
          <tiny-form-item :label="$t('sys.org.field.parent.name')">
            <tiny-input v-model="formData.id" v-show="false"></tiny-input>
            <OrgSelect v-model="formData.pid" />
          </tiny-form-item>
        </tiny-col>
      </tiny-row>
      ...
      <tiny-row>
        <tiny-col :span="6">
          <tiny-form-item :label="$t('sys.org.field.name')" :show-message="false" prop="name">
            <tiny-input v-model="formData.name"></tiny-input>
          </tiny-form-item>
        </tiny-col>
        <tiny-col :span="6">
          <tiny-form-item :label="$t('sys.org.field.sort')">
            <tiny-numeric type="number" v-model="formData.sort" mouse-wheel="true" min="0"
...
const formRef = ref()
const formData = defineModel<OrgInfo>('formData', { default: {} })
...
const handleSave = () => {
  formRef.value
    .validate()
    .then(async () => {
      await SysOrgService.save(formData.value)
      Modal.message({
